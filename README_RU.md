...
Эта версия проекта использует файловое хранение. Для разработки хранения в Microsoft SQL Server выделен отдельный проект-ответвление.
...
- Отчет содержит версию ОС, номер сборки, архитектуру, производителя, модель, серийный номер, версию Office, факты активации и список установленного ПО.
...
4. Выдайте целевым компьютерам право записи в папку `Logs` внутри пакета, если нужны центральные логи установки.
...
Скрипт развертывания пишет:

- Локальный лог: `C:\ProgramData\WindowsLicenseInventory\Logs\gpo-deploy.log`
- Центральный лог: `Logs\<COMPUTERNAME>.log` внутри папки пакета
...
Web-интерфейс содержит две вкладки:
...
- Basic Auth защищает доступ через браузер, но plain HTTP не шифрует учетные данные. Используйте HTTPS termination или ограничьте доступ доверенными сетями администрирования.
...
- Ограничьте право записи в папку `Logs` GPO-пакета только нужными учетными записями компьютеров.
